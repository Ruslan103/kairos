...
class ImportDataUseCase(private val repository: KanbanRepository) {
    suspend operator fun invoke(data: KairosTransferData): Int {
        ...
        if (defaults.size > 1) {
            val keep = defaults.find { it.name.contains("OKR", ignoreCase = true) } ?: defaults.first()
        ...
        if (active.none { it.isDefault }) {
            val next = active.firstOrNull { it.name.contains("OKR", ignoreCase = true) }
                ?: active.firstOrNull()
                ?: boards.firstOrNull()
            next?.let { repository.setDefaultBoard(it.id) }
        } else if (boards.count { it.isDefault } > 1) {
            val keep = active.find { it.isDefault && it.name.contains("OKR", ignoreCase = true) }
